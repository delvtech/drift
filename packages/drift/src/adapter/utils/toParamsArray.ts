...
/**
 * An internal version of {@linkcode toParamsArray} that includes the
 * {@linkcode AbiEntry} in the return object.
 * @internal
 */
export function _toParamsArray<
  TAbi extends Abi,
  TItemType extends AbiItemType,
  TName extends AbiEntryName<TAbi, TItemType>,
  TParameterKind extends AbiParameterKind,
...
>({
  ...
  if (!matches.length) {
    handleError(
      new DriftError(`No matching ABI entry for ${type} ${name} with ${kind}`),
    );
  }
