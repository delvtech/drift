...
- **⚡ Automatic Optimization:** Built-in caching and multicall reduce network
  requests
- **🛡️ Type Safety:** Full TypeScript support with compile-time contract
  validation
...
The Drift Client (`createDrift()`) is the orchestrator that coordinates all
operations:
...
- [**Extending Drift**](/advanced/extending-drift) - Build custom adapters and
  stores
