  ...
  { to: "0x..", amount: BigInt(100e18) },
  {
    onMined: (receipt) => {
      if (receipt?.status === "success") {
        ...
        token.cache.invalidateRead("balanceOf", { account: "0x..." });
        token.cache.invalidateRead("balanceOf", { account: "0x..." });
...
```ts twoslash
import type { ReadWriteContract, Address, erc20 } from "@delvtech/drift";
  ...
  return token.write(
    ...
    { to, amount },
    {
      onMined: async (receipt) => {
        // Precise - invalidate specific reads
...
See [**Caching**](/advanced/caching) for more details on cache management.
...
export async function safeTokenTransfer(
...
>({ token, fn, args, maxRetries = 3, options }: WriteTokenWithRetryParams<F>) {
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      console.log(
        `Failed attempt ${i + 1}/${maxRetries} for "${fn}" on ${
          token.address
        }:`,
        error,
      );
...
- [**React Integration**](/integrations/react-query) - Use writes with React
  Query
