...
Control the block range for event queries using `fromBlock` and `toBlock`:
...
### 3. Leverage Contract Instances with an `epochBlock`
...
const approvals = await drift.getEvents({
  ...
  address: "0x...",
  event: "Transfer",
  // Missing fromBlock - queries from the earliest block! // [!code hl]
});
