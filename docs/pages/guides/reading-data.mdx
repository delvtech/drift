...
// ---cut---
import { encodeFunctionData, decodeFunctionReturn } from "@gud/drift";
...
By default, Drift will automatically batch multiple `read` or `call` requests
made within the same event loop tick into a single multicall request. You can
control this behavior with the `batch` and `maxBatchSize` options when creating
the Drift client.
...
declare const tokenB: Address;
// ---cut---
// These will automatically be wrapped into a single multicall request.
const [name, symbol, totalSupply, balanceOf] = await Promise.all([
  drift.read({ abi: erc20.abi, address: tokenA, fn: "name" }),
  drift.read({ abi: erc20.abi, address: tokenA, fn: "symbol" }),
  drift.read({ abi: erc20.abi, address: tokenB, fn: "name" }),
  drift.read({ abi: erc20.abi, address: tokenB, fn: "symbol" }),
]);
```
...
// [!include ~/snippets/drift.ts]
// ---cut---
import { erc20, erc4626, erc721 } from "@gud/drift";
...
const pastBalance = await drift.read({
  abi: erc20.abi,
  address: "0x...",
  fn: "balanceOf",
  args: { account: "0x..." },
...
// Contract instances accept options as the last parameter
const pastBalance2 = await token.read(
  "balanceOf",
  { account: "0x..." },
  { block: 100n }, // [!code hl]
);
...
### 1. Use `Promise.all` or `multicall` for Multiple Reads
...
// [!include ~/snippets/token.ts]
// ---cut-end---
// ✅ Single network request (via automatic batching)
