...
// Automatically wait for transaction confirmation and handle errors
drift.hooks.on("after:write", async ({ args: [params], result }) => {
  const hash = await result;
  console.log(`Waiting for ${params.fn} transaction: ${hash}`);

  const receipt = await drift.waitForTransaction({ hash });
  if (!receipt) {
    throw new Error(`Timeout waiting for ${params.fn} transaction: ${hash}`);
  }
  if (receipt.status !== "success") {
    throw new Error(`${params.fn} transaction failed: ${hash}`);
  }

  console.log(`${params.fn} transaction confirmed!`);
});
```
...
drift.hooks.on("after:write", async ({ args: [params], result }) => {
  try {
    ...
    if (receipt?.status !== "success") {
      ...
      const error = new Error(`Transaction failed: ${params.fn}`) as any;
      error.hash = hash;
      error.params = params;
...
Drift clients provide type-safe `before:<method>` and `after:<method>` hooks for
**all** methods and function properties on the `Adapter` it uses, including
custom methods.
